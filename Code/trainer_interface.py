...
def note_handler(msg: mido.Message) -> None:
    """
    Midi message event handler
    """
    if msg.type not in ['clock', 'start', 'continue', 'stop']:
        if msg.type == "note_on" and msg.velocity == 0:
            note = mido.Message('note_off', note=msg.note, channel=msg.channel, time=msg.time)
        else:
            note = msg
...
try:
    ...
    portname = "Piaggero 0"  # replace with your MIDI INPUT
